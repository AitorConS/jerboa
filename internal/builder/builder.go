...
func (p *PythonDriver) Build(ctx context.Context, dir string, opts Options) (BuildResult, error) {
	...
	if reqData, err := os.ReadFile(filepath.Join(dir, "requirements.txt")); err == nil {
		...
		if prev, err := os.ReadFile(stampPath); err != nil || strings.TrimSpace(string(prev)) != stamp {
			// A stamped directory was generated by this driver. Remove old
			// architecture/version dependencies before pip installs replacements.
			if _, err := os.Stat(stampPath); err == nil {
				if err := os.RemoveAll(filepath.Join(dir, "packages")); err != nil {
					return BuildResult{}, fmt.Errorf("clear old Python dependencies: %w", err)
				}
			}
