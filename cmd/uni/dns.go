...
func newDNSCmd(socketPath, outputFmt *string) *cobra.Command {
	...
	cmd.AddCommand(newDNSResolveCmd(socketPath, outputFmt), newDNSListCmd(socketPath, outputFmt), newDNSResolveAllCmd(socketPath, outputFmt))
	return cmd
}
...
func newDNSResolveAllCmd(socketPath, outputFmt *string) *cobra.Command {
	var network string
	cmd := &cobra.Command{
		Use:   "resolve-all <name>",
		Short: "Resolve all IPs for a service/VM name (round-robin DNS)",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			client, err := api.Dial(*socketPath)
			if err != nil {
				return fmt.Errorf("dns resolve-all: connect to daemon: %w", err)
			}
			defer func() {
				_ = client.Close()
			}()

			recs, err := client.DNSResolveAll(cmd.Context(), args[0], network)
			if err != nil {
				return fmt.Errorf("dns resolve-all: %w", err)
			}
			if *outputFmt == "json" {
				return printJSON(cmd.OutOrStdout(), recs)
			}
